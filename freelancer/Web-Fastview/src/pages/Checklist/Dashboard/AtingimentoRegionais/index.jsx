...
export default function AtingimentoRegionais({ filters, callback }) {
    // ESTADOS
    ...
    return (
        <Col>
            <Dashboard
                id="atingimento_regionais"
                thead={
                    <Tr>
                        <Th style={{ width: 1 }}>Rede</Th>
                ...
                cols={
                    {
                        col_1: {
                            title: 'Ating. Regionais - ' + get_date('month_name', get_date('date', '01/' + filters?.monthSelected + '/' + filters?.yearSelected, 'date_sub_month', 0)),
                            api: {
                                url: window.host + '/systems/checklist/api/lista.php',
                                params: {
                                    do: 'get_regionais_dashboard',
                                    filter_id_categoria: filters?.categoriaActive,
                                    filter_id_subcategoria: filters?.subcategoriaActive,
                                    filtro_mes: filters?.monthSelected ? `${data_aux0[1]}/${data_aux0[0]}` : null,
                                    reload: (filters.yearSelected + filters.monthSelected + filters.categoriaActive + filters?.subcategoriaActive + JSON.stringify(order)),
                                    order_by: order
                                }
                            },
                            tbody: (
                                dash1.length > 0 ?
                                    dash1.map((item) => {
                                        return (
                                            ...
                                            // active={(filters?.supervisorActive == item.id_supervisor_loja ? true : false)}                                                        
                                            >
                                                <Td
                                                    onClick={() => handleClickSupervisor(filters?.supervisorActive == item.id_supervisor_loja ? '' : item.id_supervisor_loja)}
                                                // cursor="pointer"
                                                >
                                                    {item.empreendimento}
                                                </Td>
                                                ...
                                                >{porcentagem(Number(item.qtd_sim), Number(item.qtd_sim) + Number(item.qtd_nao))}</Td>
                                                ...
                                                >{porcentagem(Number(item.qtd_nao), Number(item.qtd_sim) + Number(item.qtd_nao))}</Td>
                                            </Tr>
                                        )
                                    })
                                    :
                        ...
                        col_2: {
                            title: 'Ating. Regionais - ' + get_date('month_name', get_date('date', '01/' + filters?.monthSelected + '/' + filters?.yearSelected, 'date_sub_month', 1)),
                            api: {
                                url: window.host + '/systems/checklist/api/lista.php',
                                params: {
                                    do: 'get_regionais_dashboard',
                                    ...
                                    dash2.map((item) => {
                                        return (
                                            <Tr key={item.supervisor_id}>
                                                ...
                                                <Td align="center" //cursor="pointer" onClick={() => handleClickNumber(item.id_supervisor_loja, undefined, undefined, undefined, undefined, undefined, 1)}
                                                >{porcentagem(Number(item.qtd_nao), Number(item.qtd_sim) + Number(item.qtd_nao))}</Td>
                                                {/* <Td align="center" cursor="pointer" onClick={() => handleClickNumber(item.id_supervisor_loja, undefined, undefined, undefined, undefined, undefined, 3)}>{item.qtd_feito_com_atraso}</Td> */}
                                                {/* <Td align="center" cursor="pointer" onClick={() => handleClickNumber(item.id_supervisor_loja, undefined, undefined, undefined, undefined, undefined, 2)}>{item.qtd_nao_tem}</Td> */}
                                            </Tr>
                                        )
                                    })
                                    :
                        ...
                        col_3: {
                            title: 'Ating. Regionais - ' + get_date('month_name', get_date('date', '01/' + filters?.monthSelected + '/' + filters?.yearSelected, 'date_sub_month', 2)),
                            api: {
                                url: window.host + '/systems/checklist/api/lista.php',
                                params: {
                                    do: 'get_regionais_dashboard',
