...
export default function Subcategorias({ filters, callback }) {
    // ESTADOS
        ...
        <Col>
            <Dashboard
                ...
                cols={
                    {
                        col_1: {
                            title: 'Subcategoria - ' + get_date('month_name', get_date('date', '01/' + filters?.monthSelected + '/' + filters?.yearSelected, 'date_sub_month', 0)),
                            api: {
                                url: window.host + '/systems/checklist/api/lista.php',
                                params: {
                                    do: 'get_subcategoria_dashboard',
                                    filter_id_categoria: filters?.categoriaActive,
                                    filtro_mes: filters?.monthSelected ? `${data_aux0[1]}/${data_aux0[0]}` : null,
                                    reload: (filters.yearSelected + filters.monthSelected + filters.categoriaActive + JSON.stringify(order)),
                                    order_by: order
                                    // reload: (filters?.filterModule + filters?.monthSelected + filters?.yearSelected + filters?.categoriaActive),
                                    // order_by: order
                                }
                            },
                            tbody: (
                                dash1.length > 0 ?
                                    dash1.map((item) => {
                                        return (
                                            ...
                                            >
                                                <Td
                                                    onClick={() => callback(filters?.subcategoriaActive == item.id_subcategoria ? '' : item.id_subcategoria)}
                                                    cursor="pointer"
                                                >
                                                    {item.subcategoria}
                                                </Td>
                        ...
                        col_2: {
                            title: 'Subcategoria - ' + get_date('month_name', get_date('date', '01/' + filters?.monthSelected + '/' + filters?.yearSelected, 'date_sub_month', 1)),
                            api: {
                                url: window.host + '/systems/checklist/api/lista.php',
                                params: {
                                    do: 'get_subcategoria_dashboard',
                        ...
                        col_3: {
                            title: 'Subcategoria - ' + get_date('month_name', get_date('date', '01/' + filters?.monthSelected + '/' + filters?.yearSelected, 'date_sub_month', 2)),
                            api: {
                                url: window.host + '/systems/checklist/api/lista.php',
                                params: {
                                    do: 'get_subcategoria_dashboard',
