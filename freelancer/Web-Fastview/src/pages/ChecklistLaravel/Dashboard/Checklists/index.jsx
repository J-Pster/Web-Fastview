...
export default function Checklists({filter, handleSetFilterDashboard}){
    // ESTADOS
    ...
    let url_aux = window.backend + '/api/v1/checklists/relatorios/checklists';
    ...
    if(filter?.periodo == 1){ // SE O FILTRO FOR POR MÊS
        ...
        date_start_aux_2 = get_date('date_sql', cd(filter?.data_inicio), 'date_sub_month', 1);
        date_start_aux_3 = get_date('date_sql', cd(filter?.data_inicio), 'date_sub_month', 2);
        date_end_aux_1 = get_date('date_sql', cd(filter?.data_fim));
        date_end_aux_2 = get_date('date_sql', cd(filter?.data_fim), 'date_sub_month', 1);
        date_end_aux_3 = get_date('date_sql', cd(filter?.data_fim), 'date_sub_month', 2);
    }
    ...
    return(
        <Col>
            <Dashboard
                ...
                thead={
                    <Tr>
                        <Th title="Checklist">Checklist</Th>
                    ...
                    col_1: {
                        ...
                        api: {
                            ...
                            params: {
                                ...
                                reload: (filter?.empreendimento + filter?.dashboard + filter?.periodo + filter?.supervisor + filter?.loja + filter?.funcionario + filter?.mes + filter?.ano + filter?.data_inicio + filter?.data_fim),
                            },
                            key_aux: ['data']
                        },
                        tbody: (
                                ...
                                dash1.map((item) => {
                                    return (
                                        <Tr
                                            ...
                                            active={(filter?.checklist == item.checklist_id ? true : false)}
                                            onClick={() => (
                                                handleSetFilterDashboard({tipo:'checklist', valor:item.checklist_id})
                                            )}
                                            cursor="pointer"
                                        >
