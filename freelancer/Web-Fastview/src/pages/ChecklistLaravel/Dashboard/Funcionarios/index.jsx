...
export default function Funcionarios({filter, callback, handleSetFilterDashboard}){
    // ESTADOS
    ...
    let url_aux = window.backend + '/api/v1/checklists/relatorios/funcionarios';
    ...
    return(
        <Col>
            <Dashboard
                ...
                thead={
                    <Tr>
                        <Th title="Funcionários">Funcionários</Th>
                ...
                cols={{
                    col_1: {
                        title: 'Funcionários - ' + title_aux_1,
                        api: {
                            url: url_aux,
                            params: {
                                ...
                                reload: (filter?.empreendimento + filter?.dashboard + filter?.periodo + filter?.supervisor + filter?.loja + filter?.mes + filter?.ano + filter?.data_inicio + filter?.data_fim),
                            },
                            key_aux: ['data']
                        },
                        tbody: (
                                ...
                                dash1.map((item) => {
                                    return (
                                        <Tr
                                            ...
                                            active={(filter?.funcionario == item.funcionario_id ? true : false)}
                                            onClick={() => (
                                                handleSetFilterDashboard({tipo:'funcionario', valor:item.funcionario_id})
                                            )}
                                            cursor="pointer"
                                        >
                                            <Td>{item.funcionario_nome}</Td>
                                            <Td align="center">{item.partipacao}</Td>
                                            <Td align="center" cursor="pointer" onClick={() => handleSetItem(item.funcionario_id, item.funcionario_nome, 1)}>{item.qtd_sim}</Td>
                                            <Td align="center" cursor="pointer" onClick={() => handleSetItem(item.funcionario_id, item.funcionario_nome, 2)}>{item.qtd_nao}</Td>
                                            <Td align="center" cursor="pointer" onClick={() => handleSetItem(item.funcionario_id, item.funcionario_nome, 3)}>{item.qtd_nao_aplica}</Td>
                    ...
                    col_2: {
                        title: 'Funcionários - ' + title_aux_2,
                        api: {
                            url: url_aux,
                            params: {
                    ...
                    col_3: {
                        title: 'Funcionários - ' + title_aux_3,
                        api: {
                            url: url_aux,
                            params: {
