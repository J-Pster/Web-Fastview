...
export default function Lojas({filter, callback, handleSetFilterDashboard}){
    // ESTADOS
    ...
    let url_aux = window.backend + '/api/v1/checklists/relatorios/lojas';
                    ...
                    col_1: {
                        ...
                        api: {
                            ...
                            params: {
                                ...
                                reload: (filter?.empreendimento + filter?.dashboard + filter?.periodo + filter?.supervisor + filter?.mes + filter?.ano + filter?.data_inicio + filter?.data_fim),
                            },
                            key_aux: ['data'],
                        },
                        tbody: (
                            dash1.length > 0 ?
                                dash1.map((item) => {
                                    return (
                                        <Tr
                                            key={item.loja_id}
                                            active={(filter?.loja == item.loja_id ? true : false)}
                                            onClick={() => (
                                                handleSetFilterDashboard({tipo:'loja', valor:item.loja_id})
                                            )}
                                            cursor="pointer"
                                        >
                                            <Td>{item.loja_nome}</Td>
                                            <Td align="center">{item.partipacao}</Td>
                                            <Td align="center" cursor="pointer" onClick={() => handleSetItem(item.loja_id, item.loja_nome, 1)}>{item.qtd_sim}</Td>
                                            <Td align="center" cursor="pointer" onClick={() => handleSetItem(item.loja_id, item.loja_nome, 2)}>{item.qtd_nao}</Td>
                                            <Td align="center" cursor="pointer" onClick={() => handleSetItem(item.loja_id, item.loja_nome, 3)}>{item.qtd_nao_aplica}</Td>
                    ...
                    col_2: {
                        ...
                        api: {
                            ...
                            params: {
                                ...
                                empreendimentos: filter?.empreendimento,
                                filtro_periodo: filter?.periodo,
                                filtro_data_inicio: date_start_aux_2,
                                filtro_data_fim: date_end_aux_2,
                                supervisores: (filter?.supervisor?[filter?.supervisor]:null),
                                reload: (filter?.empreendimento + filter?.dashboard + filter?.periodo + filter?.supervisor + filter?.mes + filter?.ano + filter?.data_inicio + filter?.data_fim),
                            },
                            key_aux: ['data']
                    ...
                    col_3: {
                        ...
                        api: {
                            ...
                            params: {
                                ...
                                empreendimentos: filter?.empreendimento,
                                filtro_periodo: filter?.periodo,
                                filtro_data_inicio: date_start_aux_3,
                                filtro_data_fim: date_end_aux_3,
                                supervisores: (filter?.supervisor?[filter?.supervisor]:null),
                                reload: (filter?.empreendimento + filter?.dashboard + filter?.periodo + filter?.supervisor + filter?.mes + filter?.ano + filter?.data_inicio + filter?.data_fim),
                            },
                            key_aux: ['data']
