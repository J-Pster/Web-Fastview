...
export default function Secoes({filter, handleSetFilterDashboard}){
    // ESTADOS
    ...
    let url_aux = window.backend + '/api/v1/checklists/relatorios/secoes';
                    ...
                    col_1: {
                        ...
                        api: {
                            ...
                            params: {
                                ...
                                reload: (filter?.empreendimento + filter?.dashboard + filter?.periodo + filter?.supervisor + filter?.loja + filter?.funcionario + filter?.checklist + filter?.mes + filter?.ano + filter?.data_inicio + filter?.data_fim),
                            },
                            key_aux: ['data']
                        },
                        tbody: (
                            dash1.length > 0 ?
                                dash1.map((item) => {
                                    return (
                                        <Tr
                                            key={item.secao_id}
                                            active={(filter?.secao == item.secao_id ? true : false)}
                                            onClick={() => (
                                                handleSetFilterDashboard({tipo:'secao', valor:item.secao_id})
                                            )}
                                            cursor="pointer"
                                        >
