...
import MotivoChecklist from '../../../motivo.jsx';
...
export default function Foto(props){
    ...
    // SETA STATUS INICIAL DO ESTADO "STATUSSUP"
    const statusSupInitial = () => {
        if(props?.aproved?.adm || props?.reproved?.adm || props?.inapplicable?.adm){
            if(props?.aproved?.adm){
                return 1;
            }else if(props?.reproved?.adm){
                return 2;
            }else if(props?.inapplicable?.adm){
                return 3;
            }
        }else{
            return null;
    ...
    const [statusSup, setStatusSup] = useState(statusSupInitial);
    ...
    function set_supervisao(){
        ...
        axios({
            ...
            data: {
                checklist: props.params?.checklist_id,
                pergunta: props.params?.pergunta_id,
                loja: props.params?.loja_id,
    ...
    function post_avaliacao(avaliacaoValue){
        setStatusSup(avaliacaoValue);

        axios({
            method: 'post',
            url: window.backend+'/api/v1/checklists/supervisao/avaliacao',
            data: {
                relatorio_id: props?.params?.relatorio_id,
                pergunta_id: props?.params?.pergunta_id,
    ...
    const handleCloseModalMotivo = (e) => {
        props.callback(true);

        setStatusSup(e?.status);
    ...
    return(
        <>
            ...
            <MotivoChecklist
                ...
                api={(props.params?.resposta_id && props.params?.resposta ? window.backend+'/api/v1/checklists/supervisao/avaliacao' : '')}
                show={showMotivo}
                        ...
                        {(props?.modelo === false ? // SE MODELO FOR FALSE, APARECE BOX PARA CADASTRAR FOTO
                            <>
                                <div
                                    ...
                                    style={ (imgLeft?.length > 0 ? { backgroundImage:'url('+imgLeft+')' } : { backgroundImage:''}) }
                                                ...
                                                <SwiperSlide key={'image_swiper_'+i}>
                                                    <div className="position-relative">
                                                        ...
                                                        <div
                                                            ...
                                                            style={ (imgLeft ? { backgroundImage:'url('+image+')' } : {}) } onClick={() => handleClickLeft(i)}
                                                        ></div> 
                                                    </div>
                                                </SwiperSlide>
                                            )
                                        ...
                                        {[...Array(props?.qtd)].map((foto, i) => (
                                            <SwiperSlide key={props?.params?.pergunta_id+'_foto_'+i}>
                                                <div
                                                    ...
                                                    style={ (imgRight[i] ? { backgroundImage:'url('+imgRight[i]+')' } : { backgroundImage:'url('+(imgLeft?.length > 1 ? imgLeftSelected : (imgLeft ? imgLeft[0] : ''))+')', opacity: .2 }) }
                                        ...
                                        {(() => {
                                            if(props?.rate?.adm){
                                                return(
                                                    <>
                                                        {/* CASO A FOTO ESTEJA REPROVADA OU NÃO SE APLICA, NÃO APARECE ICONE DE APROVAR/APROVADO */}
                                                        {(statusSup == 2 || statusSup == 3 || props?.interaction === false ? '' : 
                                                            <Icon
                                                                type="aprovar"
                                                                readonly={(statusSup == 1 ? true : false)}
                                                                title={(statusSup == 1 ? 'Aprovado' : 'Aprovar')}
                                                                className={(statusSup == 1 ? 'text-success' : '')}
                                                        ...
                                                        {/* CASO A FOTO ESTEJA APROVADA OU NÃO SE APLICA NÃO APARECE ICONE DE REPROVAR/REPROVADO */}
                                                        {(statusSup == 1 || statusSup == 3 || props?.interaction === false ? '' : 
                                                            <Icon
                                                                type="reprovar"
                                                                readonly={(statusSup == 2 ? true : false)}
                                                                title={(statusSup == 2 ? 'Reprovado' : 'Reprovar')}
                                                                className={(statusSup == 2 ? 'text-danger' : '')}
                                                        ...
                                                        {/* CASO A FOTO ESTEJA REPROVADA OU APROVADA, NÃO APARECE ICONE DE APROVAR/APROVADO */}
                                                        {(statusSup == 1 || statusSup == 2 || props?.interaction === false || props?.rate?.supervisao?.deactiveBan === true ? '' : 
                                                            <Icon
                                                                type="ban"
                                                                readonly={(statusSup == 3 ? true : false)}
                                                                title={(statusSup == 3 ? 'Não se aplica' : 'Não se aplica')}
                                                                className={(statusSup == 3 ? 'text-warning' : '')}
                        ...
                        if(props.date_lojista || props.date_adm || props.avaliation || props.observation || props.avaliation_observation || props.description || props?.aproved?.lojista || props?.reproved?.lojista || props?.inapplicable?.lojista){
                            return(
                                <div className={ style.foto__info + ' flex-1 '+(props.description ? 'text-start' : 'text-start') }>   
                                    ...
                                    {(statusSup == 1 || statusSup == 2 || statusSup == 3 ?
                                        <div className="d-block">
                                            <b>Supervisor:</b> {props?.adm}
                                            {(statusSup == 1 ? ' / Aprovado' : '')}
                                            {(statusSup == 2 ? ' / Reprovado'+(props.motive_supervisor ? ' ('+props.motive_supervisor+')':'') : '')}
                                            {(statusSup == 3 ? ' / Não se aplica'+(props.motive_supervisor ? ' ('+props.motive_supervisor+')':'') : '')}
