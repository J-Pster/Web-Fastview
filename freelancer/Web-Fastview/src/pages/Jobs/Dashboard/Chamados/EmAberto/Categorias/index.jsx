...
export default function Categorias({chartOptions, filters}){
    ...
    useEffect(() => {
        ...
        axios({
            ...
            if(response?.data){
                let data_aux = [['Categoria', 'Chamados']];
                let total_aux = 0;

                response?.data?.map((item, i) => {
                    data_aux.push([item?.categoria_nome, parseInt(item?.qtd)]);
    ...
    return(
        <Box
            title={<><span className="text-primary font-weight-bold">Chamados</span> por Categoria</>}
            loading={loading}
            body={
                <>
