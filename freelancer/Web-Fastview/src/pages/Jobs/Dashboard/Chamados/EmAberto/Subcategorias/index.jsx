...
export default function Subcategorias({chartOptions, filters}){
    ...
    useEffect(() => {
        ...
        axios({
            ...
            if(response?.data){
                let data_aux = [['Subcategoria', 'Chamados']];
                let total_aux = 0;

                response?.data?.map((item, i) => {
                    data_aux.push([item?.subcategoria_nome, parseInt(item?.qtd)]);
    ...
    return(
        <Box
            title={<><span className="text-primary font-weight-bold">Chamados</span> por Subcategoria</>}
            loading={loading}
            body={
                <>
