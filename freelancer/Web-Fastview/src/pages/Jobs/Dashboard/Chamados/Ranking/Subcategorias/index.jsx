...
export default function Subcategorias({chartOptions, filters, callback}){
    // ESTADOS
        ...
        axios({
            ...
            if(response?.data){
                ...
                response?.data?.map((item, i) => {
                    data_aux.push([(item?.subcategoria_descricao?.slice(0,15)+'...'), parseInt(item?.qtd_inferior), parseInt(item?.qtd_superior)]);
                });
                ...
                title={
                    <>
                        ...
                        <span className="ms-1 cursor-pointer" onClick={handleChange}>
                            <span className="text-primary font-weight-bold">Subcategorias</span> <Icon type="change" className="mx-1" title={false} readonly /> Categorias
                        </span>
                    </>
                }
