...
export default function Departamentos({chartOptions, filters}){
        ...
        axios({
            ...
            if(response?.data){
                ...
                response?.data?.map((item, i) => {
                    data_aux.push([item?.modulo_descricao, parseInt(item?.qtd_inferior), parseInt(item?.qtd_superior)]);
                });
    ...
    return(
        <Box
            title={<span className="text-primary font-weight-bold">SLA por Departamento</span>}
            subtitle={'SLA 7 dias úteis'}
            className={'mb-0 '+style.borderBottomRadiusNone + ' ' + style.borderBottomNone}
            loading={loading}
            body={
                <Chart
                    chartType="BarChart"
