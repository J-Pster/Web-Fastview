...
export default function Subcategorias({chartOptions, filters}){
    // ESTADOS
        ...
        axios({
            ...
            if(response?.data){
                ...
                response?.data?.map((item, i) => {
                    data_aux.push([item?.subcategoria_descricao, parseInt(item?.qtd_inferior), parseInt(item?.qtd_superior)]);
                });
    ...
    return(
        <Box
            title={<span className="text-primary font-weight-bold">SLA por Subcategoria</span>}
            subtitle={'SLA 7 dias úteis'}
            className={'mb-0 ' + style.borderTopRadiusNone}
            loading={loading}
            body={
                <Chart
                    chartType="BarChart"
