...
export default function Categorias({filters, callback, chamados}){
    // ESTADOS
    ...
    // GET RESULTADOS DA PRIMEIRA COLUNA DE CATEGORIAS
    ...
    // GET RESULTADOS DA SEGUNDA COLUNA DE CATEGORIAS
    ...
    // GET RESULTADOS DA TERCEIRA COLUNA DE CATEGORIAS
    ...
    // CLICK CATEGORIAS
    function handleClickCategoria(categoria_id){
        if(callback){
            callback({
                getInfo: undefined,
                filterCol: {
        ...
        <Col>
            <Dashboard
                ...
                cols={
                    {
                        col_1: {
                            title: 'Categorias - '+get_date('month_name',get_date('date','01/'+filters?.monthSelected+'/'+filters?.yearSelected,'date_sub_month',0)),
                            api: {
                                ...
                                params: {
                                    ...
                                    filter_active_2: [0, 1],
                                    reload: (filters?.filterModule + filters?.monthSelected + filters?.yearSelected + filters?.supervisorActive + filters?.lojaActive + filters?.usuarioActive + JSON.stringify(order)),
                                    order_by: order,
                                ...
                                dash1.length>0?
                                    dash1.map((item) => {
                                        return(
                                            ...
                                            >
                                                <Td
                                                    onClick={() => handleClickCategoria(filters?.categoriaActive == item.id_categoria ? '' : item.id_categoria)}
                                                    cursor="pointer"
                                                >
                                                    {item.nome_categoria}
                                                </Td>
                                                ...
                                                {(!chamados ?
                                                    <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, undefined, undefined, item.id_categoria, undefined, undefined, -2)}>{item.qtd_atrasado}</Td>
                                                :'')}

                                                <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, undefined, undefined, item.id_categoria, undefined, undefined, 1)}>{item.qtd_feito}</Td>

                                                {(!chamados ?
                                                    <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, undefined, undefined, item.id_categoria, undefined, undefined, 3)}>{item.qtd_feito_com_atraso}</Td>
                                                :'')}
                                                <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, undefined, undefined, item.id_categoria, undefined, undefined, 2)}>{item.qtd_nao_tem}</Td>
                                            </Tr>
                                        )
                                    })
                                :
                        ...
                        col_2: {
                            title: 'Categorias - '+get_date('month_name',get_date('date','01/'+filters?.monthSelected+'/'+filters?.yearSelected,'date_sub_month',1)),
                            api: {
                                url: window.host_madnezz+'/systems/integration-react/api/request.php',
                                params: {
                                    db_type: global.db_type,
                        ...
                        col_3: {
                            title: 'Categorias - '+get_date('month_name',get_date('date','01/'+filters?.monthSelected+'/'+filters?.yearSelected,'date_sub_month',2)),
                            api: {
                                url: window.host_madnezz+'/systems/integration-react/api/request.php',
                                params: {
                                    db_type: global.db_type,
