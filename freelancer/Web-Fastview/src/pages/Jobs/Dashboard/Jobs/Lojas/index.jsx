...
export default function Lojas({filters, callback}){
    // ESTADOS
    ...
    // CLICK LOJAS
    function handleClickLoja(loja_id){
        if(callback){
            callback({
                getInfo: undefined,
                filterCol: {
        ...
        <Col>
            <Dashboard
                ...
                cols={
                    {
                        col_1: {
                            title: 'Lojas - '+get_date('month_name',get_date('date','01/'+filters?.monthSelected+'/'+filters?.yearSelected,'date_sub_month',0)),
                            api: {
                                ...
                                dash1.length>0?
                                    dash1.map((item) => {
                                        return(
                                            <Tr
                                                key={item.id_loja}
                                                active={(filters?.lojaActive == item.id_loja ? true : false)}                                                        
                                            >
                                                <Td
                                                    onClick={() => handleClickLoja(filters?.lojaActive == item.id_loja ? '' : item.id_loja)}
                                                    cursor="pointer"
                                                >
                                                    {item.nome_loja}
                                                </Td>

                                                <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, item.id_loja, undefined, undefined, undefined, undefined, -2)}>{item.qtd_atrasado}</Td>
                                                <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, item.id_loja, undefined, undefined, undefined, undefined, 1)}>{item.qtd_feito}</Td>
                                                <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, item.id_loja, undefined, undefined, undefined, undefined, 3)}>{item.qtd_feito_com_atraso}</Td>
                                                <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, item.id_loja, undefined, undefined, undefined, undefined, 2)}>{item.qtd_nao_tem}</Td>
                                            </Tr>
                                        )
                                    })
                                :
                        ...
                        col_2: {
                            title: 'Lojas - '+get_date('month_name',get_date('date','01/'+filters?.monthSelected+'/'+filters?.yearSelected,'date_sub_month',1)),
                            api: {
                                url: window.host_madnezz+'/systems/integration-react/api/request.php',
                                params: {
                                    db_type: global.db_type,
                        ...
                        col_3: {
                            title: 'Lojas - '+get_date('month_name',get_date('date','01/'+filters?.monthSelected+'/'+filters?.yearSelected,'date_sub_month',2)),
                            api: {
                                url: window.host_madnezz+'/systems/integration-react/api/request.php',
                                params: {
                                    db_type: global.db_type,
