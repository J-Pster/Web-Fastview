...
export default function Subcategorias({filters, callback, chamados}){
    // ESTADOS
    ...
    // GET RESULTADOS DA PRIMEIRA COLUNA DE SUBCATEGORIAS
    ...
    // GET RESULTADOS DA SEGUNDA COLUNA DE SUBCATEGORIAS
    ...
    // GET RESULTADOS DA TERCEIRA COLUNA DE SUBCATEGORIAS
    ...
    // CLICK SUBCATEGORIAS
    function handleClickSubcategoria(subcategoria_id){
        if(callback){
            callback({
                getInfo: undefined,
                filterCol: {
        ...
        <Col>
            <Dashboard
                ...
                cols={
                    {
                        col_1: {
                            title: 'Subcategorias - '+get_date('month_name',get_date('date','01/'+filters?.monthSelected+'/'+filters?.yearSelected,'date_sub_month',0)),
                            api: {
                                ...
                                params: {
                                    ...
                                    filter_id_user: filters?.usuarioActive,
                                    filter_category: filters?.categoriaActive, 
                                    filter_active_2: [0, 1],
                                    reload: (filters?.filterModule + filters?.monthSelected + filters?.yearSelected + filters?.supervisorActive + filters?.lojaActive + filters?.usuarioActive + filters?.categoriaActive + JSON.stringify(order)),
                                    order_by: order,
                                    ...
                                    dash1.map((item) => {
                                        return(
                                            <Tr
                                                ...
                                                active={(filters?.subcategoriaActive == item.id_subcategoria ? true : false)}                                                        
                                            >
                                                <Td>
                                                ...
                                                {(!chamados ?
                                                    <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, undefined, undefined, undefined, item.id_subcategoria, undefined, -2)}>{item.qtd_atrasado}</Td>
                                                :'')}

                                                <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, undefined, undefined, undefined, item.id_subcategoria, undefined, 1)}>{item.qtd_feito}</Td>

                                                {(!chamados ?
                                                    <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, undefined, undefined, undefined, item.id_subcategoria, undefined, 3)}>{item.qtd_feito_com_atraso}</Td>
                                                :'')}

                                                <Td align="center" cursor="pointer" onClick={() => handleClickNumber(undefined, undefined, undefined, undefined, item.id_subcategoria, undefined, 2)}>{item.qtd_nao_tem}</Td>
                                            </Tr>
                                        )
                                    })
                                :
                        ...
                        col_2: {
                            title: 'Subcategorias - '+get_date('month_name',get_date('date','01/'+filters?.monthSelected+'/'+filters?.yearSelected,'date_sub_month',1)),
                            api: {
                                url: window.host_madnezz+'/systems/integration-react/api/request.php',
                                params: {
                                    db_type: global.db_type,
                        ...
                        col_3: {
                            title: 'Subcategorias - '+get_date('month_name',get_date('date','01/'+filters?.monthSelected+'/'+filters?.yearSelected,'date_sub_month',2)),
                            api: {
                                url: window.host_madnezz+'/systems/integration-react/api/request.php',
                                params: {
                                    db_type: global.db_type,
